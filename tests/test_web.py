...
""".strip(),
            ...
            self.assertNotIn("patterns", state["market"])
            self.assertNotIn("sample_plan", state["market"])
            self.assertNotIn("opportunity_radar", state)
            self.assertNotIn("brand_weight_profile", state)
            self.assertNotIn("market_alerts", state)
            self.assertNotIn("focus_queue", state)
    ...
    def test_feed_index_html_is_feed_app(self) -> None:
        ...
        self.assertNotIn("new_release:", FEED_INDEX_HTML)
        ...
        self.assertIn(".visual.drop.has-image img", FEED_INDEX_HTML)
        self.assertIn("object-fit: contain", FEED_INDEX_HTML)
        self.assertIn('referrerpolicy="no-referrer"', FEED_INDEX_HTML)
        self.assertIn("fallbackVisual(this)", FEED_INDEX_HTML)
        self.assertIn(".feed-card.trend-card", FEED_INDEX_HTML)
        self.assertIn(".trend-card .visual.trend", FEED_INDEX_HTML)
        self.assertIn("grid-template-columns: minmax(0, 1fr)", FEED_INDEX_HTML)
        self.assertIn("titleAltHtml(row)", FEED_INDEX_HTML)
        self.assertIn("titleText(row)", FEED_INDEX_HTML)
        self.assertIn("localizedTitle(row)", FEED_INDEX_HTML)
        self.assertIn("row.use_localized_title", FEED_INDEX_HTML)
        self.assertIn('if (language === "zh" && localized) return localized;', FEED_INDEX_HTML)
        self.assertIn("TEXT[language].original", FEED_INDEX_HTML)
        self.assertNotIn("`${localized} · ${row.title}`", FEED_INDEX_HTML)
        self.assertIn("detailHtml(row)", FEED_INDEX_HTML)
        self.assertIn("sourceContextHtml(row)", FEED_INDEX_HTML)
        self.assertIn("row.source_context", FEED_INDEX_HTML)
        self.assertIn("source-context", FEED_INDEX_HTML)
        ...
        self.assertIn("源头发布时间", FEED_INDEX_HTML)
        self.assertIn("掲載元日", FEED_INDEX_HTML)
        self.assertIn("langBtn", FEED_INDEX_HTML)
        self.assertIn("ロリィタ発売情報", FEED_INDEX_HTML)
        self.assertIn('if (language === "zh" && localized) return localized;', FEED_INDEX_HTML)
        self.assertIn("<article class=\"feed-card", FEED_INDEX_HTML)
        self.assertIn('<a class="cta" href="${escapeHtml(row.url)}"', FEED_INDEX_HTML)
        self.assertNotIn('const tag = hasUrl ? "a" : "article"', FEED_INDEX_HTML)
        self.assertIn("打开来源", FEED_INDEX_HTML)
        self.assertIn("ソースを開く", FEED_INDEX_HTML)
        self.assertIn("暂无来源链接", FEED_INDEX_HTML)
        self.assertIn("brandFilter", FEED_INDEX_HTML)
        self.assertIn("platformFilter", FEED_INDEX_HTML)
        self.assertIn("stockFilter", FEED_INDEX_HTML)
        self.assertIn("watchFilter", FEED_INDEX_HTML)
        self.assertIn("renderQuickFilters(feed)", FEED_INDEX_HTML)
        self.assertIn("applyQuickFilters(rawRows)", FEED_INDEX_HTML)
        self.assertIn("trustHtml(row)", FEED_INDEX_HTML)
        self.assertIn("evidenceHtml(row)", FEED_INDEX_HTML)
        self.assertIn("actionHtml(row)", FEED_INDEX_HTML)
        self.assertIn("lolitaRadarCardState", FEED_INDEX_HTML)
        self.assertIn("confidence-bar", FEED_INDEX_HTML)
        self.assertIn("data-card-action", FEED_INDEX_HTML)
        self.assertIn("全部品牌", FEED_INDEX_HTML)
        self.assertIn("全ブランド", FEED_INDEX_HTML)
        self.assertIn("FEED_LABELS", FEED_INDEX_HTML)
        self.assertIn("FEED_LABELS[language][activeFilter]", FEED_INDEX_HTML)
        self.assertIn('activeFilter === "all"', FEED_INDEX_HTML)
        self.assertIn("feed.streams?.[activeFilter]", FEED_INDEX_HTML)
        self.assertIn('button.addEventListener("click"', FEED_INDEX_HTML)
        self.assertIn('item.classList.toggle("active"', FEED_INDEX_HTML)
...
""".strip(),
            ...
            self.assertEqual(source_alerts[0]["title_zh"], "Angelic Pretty 来源不可用")
            self.assertEqual(source_alerts[0]["title_ja"], "Angelic Pretty 取得不可")
            self.assertTrue(source_alerts[0]["use_localized_title"])
...
""".strip(),
            ...
            self.assertEqual(feed["streams"]["alert"][0]["title_zh"], "Angelic Pretty 来源不可用")
            self.assertEqual(feed["streams"]["alert"][0]["title_ja"], "Angelic Pretty 取得不可")
            self.assertTrue(feed["streams"]["alert"][0]["use_localized_title"])
